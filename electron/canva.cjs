  ...
  return getClientCredentials().then((creds) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      server = http.createServer((req, res) => {
        ...
        res.end(
          `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:1.5rem"><p><strong>Connected.</strong> You can close this tab and return to Collectif Badge Manager.</p></body></html>`
        );
        clearTimeout(timer);
        server.close(() => {
