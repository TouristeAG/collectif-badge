...
/** When launched from Finder, main-process errors are invisible — log for diagnosis. */
const MAIN_CRASH_LOG = path.join(os.homedir(), "Library", "Logs", "CollectifBadge-main.log");
function logMainCrash(label, err) {
  const body = err && err.stack ? err.stack : String(err);
  const line = `${new Date().toISOString()} [${label}]\n${body}\n\n`;
  try {
    fssync.appendFileSync(MAIN_CRASH_LOG, line);
  } catch {
    /* ignore */
  }
}
process.on("uncaughtException", (err) => {
  logMainCrash("uncaughtException", err);
});
  ...
  app.on("activate", () => {
    if (BrowserWindow.getAllWindows().length === 0) {
      try {
        ...
        logMainCrash("createWindow(activate)", err);
      }
    }
  });
});
